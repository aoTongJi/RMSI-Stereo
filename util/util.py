...
def compute_scale_and_shift(disp, eps=1e-6):
    """Affine-invariant median/MAD statistics with numerical safeguards."""
    flat_disp = disp.float().flatten(2)
    ...
    scale = torch.nan_to_num(scale, nan=1.0, posinf=1.0, neginf=1.0).clamp_min(eps)
    return scale, shift


def normalize_disparity(disp, eps=1e-6, detach_stats=False):
    """Normalize disparity/depth by median and mean absolute deviation.

    ``detach_stats=True`` implements N_sg(.) used by RAPC: prediction values
    still receive gradients, while the affine normalization statistics do not.
    """
    dtype = disp.dtype
    disp_f = disp.float()
    scale, shift = compute_scale_and_shift(disp_f, eps=eps)
    if detach_stats:
        scale = scale.detach()
        shift = shift.detach()
    norm_disp = (disp_f - shift[..., None, None]) / scale[..., None, None]
